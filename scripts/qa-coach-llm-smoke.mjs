...
const PROFILES = {
  '18h': {
    ...
    rounds: 10,
    holes: 18,
    // total_gross típico 18-hole para un 14-handicap: ~ 88 (84-92)
    grossMin: 84, grossMax: 92,
    forbiddenAvgRange: [35, 55], // si el LLM cita avg en este rango → cruzó con 9h
    expectedAvgRange: [82, 96],
  },
  '9h': {
    ...
    rounds: 10,
    holes: 9,
    grossMin: 42, grossMax: 48, // avg ~ 45
    forbiddenAvgRange: [70, 110], // si cita avg aquí → cruzó con 18h
    expectedAvgRange: [40, 52],
  },
  'mixed': {
    ...
    rounds18: 5,
    rounds9: 5,
    gross18Min: 85, gross18Max: 92,
    gross9Min: 42, gross9Max: 48,
    // mustMentionBoth: respuesta debe citar ambos buckets explícitamente
    requireBucketMention: true,
  },
}
...
function assertProfile(profileName, profile, response) {
  ...
  if (profile.requireBucketMention) {
    const lower = response.toLowerCase()
    const has9 = /9\s*hoyos|9h|nueve hoyos/.test(lower)
    const has18 = /18\s*hoyos|18h|dieciocho hoyos/.test(lower)
    if (!has9 || !has18) {
      failures.push(`Mixed: respuesta no cita ambos buckets explícitamente (9h=${has9}, 18h=${has18})`)
    }
  }
