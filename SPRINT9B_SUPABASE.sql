-- ═══════════════════════════════════════════════════════════════════════════
-- SPRINT 9B — Tu Golf: SQL completamente idempotente
...
ALTER TABLE rondas_libres ADD COLUMN IF NOT EXISTS modo_juego TEXT DEFAULT 'gross' CHECK (modo_juego IN ('gross','neto','stableford'));

ALTER TABLE tournaments ADD COLUMN IF NOT EXISTS modo_juego TEXT DEFAULT 'gross' CHECK (modo_juego IN ('gross','neto','stableford'));

ALTER TABLE historical_rounds ADD COLUMN IF NOT EXISTS total_neto INTEGER;

ALTER TABLE historical_rounds ADD COLUMN IF NOT EXISTS total_stableford INTEGER;
...
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS patterns_need_recalc BOOLEAN DEFAULT FALSE;

-- ─── 2. Tabla: player_patterns ──────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_player_patterns_user ON player_patterns(user_id);
CREATE INDEX IF NOT EXISTS idx_player_patterns_type ON player_patterns(user_id, pattern_type);
CREATE INDEX IF NOT EXISTS idx_taiger_sessions_user ON taiger_sessions(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_historical_rounds_user ON historical_rounds(user_id, played_at DESC);
CREATE INDEX IF NOT EXISTS idx_hole_scores_round ON hole_scores(round_id);
CREATE INDEX IF NOT EXISTS idx_course_holes_course_numero ON course_holes(course_id, numero);
CREATE INDEX IF NOT EXISTS idx_rondas_libres_codigo ON rondas_libres(codigo);
CREATE INDEX IF NOT EXISTS idx_ronda_jugadores_ronda ON ronda_libre_jugadores(ronda_id);

-- ═══════════════════════════════════════════════════════════════════════════
-- FIN — Todo idempotente, seguro de ejecutar múltiples veces
-- ═══════════════════════════════════════════════════════════════════════════
