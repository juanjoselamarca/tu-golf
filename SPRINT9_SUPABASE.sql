-- ═══════════════════════════════════════════════════════════════════════════
-- SPRINT 9 — Tu Golf: tAIger foundation + Garmin + patterns + handicap history
...
CREATE TABLE IF NOT EXISTS taiger_sessions (
  ...
  session_type         TEXT NOT NULL
    CHECK (session_type IN ('post_round','weekly_plan',
                            'pre_tournament','onboarding')),
...
CREATE TABLE IF NOT EXISTS player_psych_profile (
  ...
  pressure_response    TEXT
    CHECK (pressure_response IN ('activacion','paralisis','mixto')),
  motivation_type      TEXT
    CHECK (motivation_type IN ('competitivo','disfrute','social')),
  primary_fear         TEXT,
...
-- ══════════════════════════════════════════════════════════════════════════
-- VERIFICACIÓN: si no hay errores arriba, el sprint 9 está listo en BD.
-- ══════════════════════════════════════════════════════════════════════════
