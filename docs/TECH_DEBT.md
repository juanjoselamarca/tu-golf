...
| P1-12 | `/ronda-libre/[codigo]/score` crashea con rondas recién creadas | ✅ Resuelto `b70c53b` (2026-05-12) | **Root cause encontrado en cancha (Juanjo jugando en Los Leones)**: `hasStrokeAdvantage` callback (introducido en `e1f3571`, 14-abr) hacía closure sobre `modoJuego`/`formatoJuego` declaradas más abajo en el mismo bloque del `ScorePageContent`. La llamada sincrónica `const strokeAdvantageOnHole = hasStrokeAdvantage(...)` accedía a `modoJuego` en TDZ → `ReferenceError: Cannot access 't$' before initialization` (`t$` = nombre minificado de `modoJuego`). TS no lo detecta porque la referencia es vía closure, no directa (TS2448 sólo cubre directa). El diagnóstico inicial de "sólo afecta admin fixture" era erróneo — afectaba a TODA ronda con `scores = {}` (sin score aún). Las rondas reales que "funcionaban" eran las que ya tenían algún hoyo anotado. Fix: mover `const modoJuego` y `const formatoJuego` ANTES de `hasStrokeAdvantage`. Canary test agregado en `canary-stability.test.ts`. Tests `e2e/ronda-scoring.spec.ts` des-skipeados en commit de seguimiento. |
