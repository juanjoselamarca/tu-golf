...
| P1-11 | `/leaderboard` hydration mismatch SSR↔CSR | ✅ Resuelto `c77373e` (2026-04-23) | Descubierto por e2e smoke. Errors #418/#423/#425 causados por `Date.now()` como seed inicial en `useDemoSimulation`. Fix: seed determinístico (INITIAL_SEED=0) + useEffect que re-siembra client-side. Verificado en prod: 0 errors React. Ciclo descubrir-fixear-verificar en una sesión. |
| P1-12 | `/ronda-libre/[codigo]/score` crashea con rondas recién creadas | 🔴 **ABIERTO** (2026-04-23) | Descubierto mientras implementaba e2e scoring UI. El bundle minificado tira `ReferenceError: Cannot access 't$' before initialization` para CUALQUIER ronda creada desde cero (mi admin fixture). Rondas reales existentes NO tienen el bug. Página principal `/ronda-libre/[codigo]` (sin /score) funciona OK para las mismas rondas. Implica un codepath específico del archivo 1947 LOC de scoring que falla con algún estado inicial. Tests `e2e/ronda-scoring.spec.ts` quedan con `test.skip` hasta que se arregle. Próximo paso: ejecutar dev local con source-maps activos para desofuscar `t$` y ubicar la línea exacta en src/app/ronda-libre/[codigo]/score/page.tsx. Alta prioridad — afecta onboarding de usuarios nuevos creando su primera ronda. Excepto que en producción nadie lo ha reportado porque todas las rondas activas se crearon via wizard real (no admin fixture) — el wizard puede estar seteando algún campo implícito que no replicamos. |
