...
- **Fase 2 — Observabilidad + alerta** ✅ (PR siguiente): tabla `ai_usage` + logging best-effort desde el gateway (provider/modelo/fallback/tokens/latencia/costo/error_kind), `evaluateAiAlerts` (fail-rate / fallback-rate / rate-limit con umbrales), enganchada al cron diario `health-check` → `captureError` + **notificación real a Telegram** (`TELEGRAM_ALLOWED_CHAT_ID`). Métricas filtradas por `ai_env='prod'`. **Diferido (follow-up chico):** widget visual en admin (la alerta proactiva por Telegram cubre el "avísanos al 70%"; el dashboard on-demand es secundario).
