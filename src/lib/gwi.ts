import type { ModoJuego } from './scoring'
...
// ─── Volatilidad por índice ───
// HCP 0 → σ 1.50 | HCP 10 → σ 2.55 | HCP 18 → σ 3.56 | HCP 28 → σ 4.90
...
// ─── Probabilidad de resultado por hoyo ───
...
export interface JugadorGWIInput {
  ...
  currentScore:          number       // overUnder gross/neto OR puntos stableford
...
export function calcularGWI(
  ...
  if (jugadores.length === 1) return [{
    ...
    tendencia:      'stable',
    volatilidad:    'baja',
    narrativa:      'Jugando solo',
    breakdown: {
      situacion:    { peso: 100, valor: 0 },
      historico:    { peso: 0, valor: 0, confianza: 0 },
      cancha:       { peso: 0, valor: 0, confianza: 0 },
      patrones:     { peso: 0, valor: 0 },
  ...
  const W1 = 0.30 + progresoRatio * 0.50  // 30% → 80%
  ...
  const ajustados = jugadores.map(j => {
    ...
    if (j.patterns?.back9Collapse && hoyosRestantes <= 9) {
      F4 += j.patterns.back9Collapse.confidence
        * j.patterns.back9Collapse.avgDiff
        * (hoyosRestantes / 9)
    }
    ...
    if (j.patterns?.courseSpecific) {
      F4 += j.patterns.courseSpecific.confidence
        * j.patterns.courseSpecific.avgDiff * 0.5
    }
    ...
    const volatilidad  = j.handicapIndex <= 6  ? 'baja'
      : j.handicapIndex <= 16 ? 'media' : 'alta'
    const sigmaLabel   = j.handicapIndex <= 6  ? 'Juego muy consistente'
      : j.handicapIndex <= 16 ? 'Varianza moderada' : 'Alto potencial de cambio'
  ...
  return ajustados.map((j, i) => {
    ...
    let narrativa = ''
    if (hoyosRestantes === 0)
      narrativa = 'Ronda finalizada'
    else if (esLider && j.handicapIndex <= 8 && hoyosRestantes <= 4)
      narrativa = 'Ventaja sólida — consistencia garantiza'
    else if (esLider && j.handicapIndex >= 18 && hoyosRestantes <= 6)
      narrativa = 'Lidera pero varianza deja puerta abierta'
    else if (!esLider && diferencia <= j.sigma * 0.5)
      narrativa = 'Dentro del margen — todo puede cambiar'
    else if (!esLider && diferencia > j.sigma * 1.2)
      narrativa = `Necesita un rallye en ${hoyosRestantes} hoyos`
    else if (j.breakdown.patrones.valor > 1.5 && hoyosRestantes <= 9)
      narrativa = 'Patrón de colapso detectado aquí'
    else if (j.breakdown.cancha.confianza > 0.7 && j.breakdown.cancha.valor < -3)
      narrativa = 'Históricamente dominante en esta cancha'
